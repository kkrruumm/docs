...
The environment variable `MESA_VK_DEVICE_SELECT_FORCE_DEFAULT_DEVICE=<ID>` should be applied if using Mesa drivers to tell the game what graphics card it should use, where `<ID>` is the identifier number of the graphics card you wish to use.

In order to get the ID numbers of your graphics cards, you may run `vulkaninfo --summary` and look for the `Devices:` section of this output. An example of this output may be:
...
In which case, the desired GPU would have an `<ID>` of `0`.
...
3. Add `MESA_VK_DEVICE_SELECT_FORCE_DEFAULT_DEVICE=<ID> %command%` to the **Launch Options** section, where `<ID>` is the ID number gathered above.
