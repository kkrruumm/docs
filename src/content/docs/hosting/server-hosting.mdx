...
:::note[Exceptions]
These may not apply to LAN servers/Users hosting privately. This mostly applies to users looking to have a public server for ALL IW4x user's.

Users hosting LAN/Privately can avoid port forwarding with VLAN softwares like Hamachi, Tailscale, RAdmin, etc.
:::
...
2. Start `DedicatedServer.bat` (for a Dedicated server) or `DedicatedLobbyServer.bat` (for a party server)
...
1. Navigate to your MW2 server folder (`C:\GameServers\MW2-Server`)
2. Start `DedicatedServer.sh` (for a Dedicated server) or `DedicatedLobbyServer.sh` (for a party server)
